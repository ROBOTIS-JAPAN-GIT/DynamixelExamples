...
void DynamixelHundler::CallbackCommand(const trajectory_msgs::JointTrajectory::ConstPtr& msg){
    ...
    for (int i=0; i<msg->joint_names.size(); i++ ) {
        ...
        for (int j = 0; j < joint_list_.size(); j++ ) {
            if (joint_list_[j] == joint_name){
                positions[j] = (int)(angle * 4096 / (2*M_PI) );
...
void DynamixelHundler::WritePosisionsAll(const std::vector<int32_t>& position_list){
  if(position_list.size() != id_list_.size()){
    ROS_ERROR("Length of position_list is mismatch to number of Dynamixel in WritePosisionsAll.");
    return;
  }
  ...
  for(size_t i = 0; i < id_list_.size(); ++i){  
    ...
    const int dxl_addparam_result = groupSyncWritePosition_.addParam(id, param_goal_position);
    if (dxl_addparam_result != true)
      ROS_ERROR( "Failed to addparam to groupSyncWrite for Dynamixel ID %d", id);
  }
...
bool DynamixelHundler::ReadPositionsAll(){
  ...
  for (size_t i = 0; i < id_list_.size(); ++i) {
    int32_t position = groupSyncReadPosition_.getData(id_list_[i], ADDR_PRESENT_POSITION, 4);
...
int main(int argc, char ** argv){
  ...
  if(!nh_p.getParam("DEVICE_NAME", DEVICE_NAME)) DEVICE_NAME = "COM3";
  if(!nh_p.getParam("BAUDRATE", BAUDRATE)      ) BAUDRATE    = 57600;
  std::vector<int> id_list = {1, 2}; // id と joint_name 辞書型にした方がいいかな．
  std::vector<std::string> joint_list = {"joint1", "joint2"};
  ...
  while(ros::ok()) {
    if( dxl_hundler.ReadPositionsAll() ){
        ...
        for (size_t i = 0; i < dxl_hundler.id_list().size(); ++i) {
            joint_state_msg.desired.positions[i] = dxl_hundler.target_positions()[i] * (2*M_PI) / 4096 ;
            joint_state_msg.actual.positions[i]  = dxl_hundler.now_positions()[i] * (2*M_PI) / 4096 ;
            joint_state_msg.error.positions[i]   = joint_state_msg.desired.positions[i]-joint_state_msg.actual.positions[i] ;
        }
